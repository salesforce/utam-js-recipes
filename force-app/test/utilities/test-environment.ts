...
export class TestEnvironment {
     ...
     * @param envNamePrefix Test environment prefix
     */
    constructor(envNamePrefix: string) {
     ...
     */
    #getKeyFromEnv(keyName: string): string {
     ...
     * @param keyName name of the key property as defined in the class constructor
     * @returns the key that matches the property in environment variables
     */
    #getEnvKeyName(keyName: string): `${string}_${string}` {
     ...
     * @param keyName name of the key property as defined in the class constructor
     * @returns return the key with the format that matches environment variables (without prefix)
     */
    #camelCaseToUpperCase(keyName: string): string {
        return keyName
            .split(/(?=[A-Z])/)
            .join('_')
            .toUpperCase();
    ...
    get baseUrl(): string {
        if (!this.#baseUrl) {
            throw new Error(`Property ${this.#getEnvKeyName('url')} is not set in '.env' property file`);
        }
        return this.#baseUrl;
    }
    ...
    get username(): string {
        if (!this.#username) {
            throw new Error(`Property ${this.#getEnvKeyName('username')} is not set in '.env' property file`);
        }
    ...
    get password(): string {
        if (!this.#password) {
            throw new Error(`Property ${this.#getEnvKeyName('password')} is not set in '.env' property file`);
        }
