...
// to run:
// yarn test --spec force-app/test/record-update.spec.ts
...
describe('Record update test', () => {
     ...
     * @param recordType type of record used in the UI test
     * @param recordId id of the record for which we are getting the URL
     */
    async function gotoRecordHomeByUrl(recordType: RecordType, recordId: string): Promise<void> {
    ...
    const equalsIgnoreCase = (str1: string, str2: string): boolean => str1.toLowerCase() === str2.toLowerCase();
